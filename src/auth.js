...
export default class DropboxAuth {
  ...
  generatePKCECodes() {
    let codeVerifier;
    if (isBrowserEnv()) {
      const array = new Uint32Array(PKCELength);
      const randomValueArray = crypto.getRandomValues(array);
      codeVerifier = convertToStringAndReplace(randomValueArray).substr(0, 128); // convert to browser safe string
    } else {
   ...
   * @returns {Promise<String>} Returns a Promise with the url string as the response
   */
